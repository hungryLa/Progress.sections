    ...
    return (
        <div>
            ...
            <div style={{overflow: 'hidden'}}>
                <table style={{overflowX: 'auto', display: 'block', whiteSpace: 'nowrap', maxWidth: '100%', width: '100%'}}>
                    <thead>
                    <tr style={{textAlign: 'left'}}>
                        <th style={{minWidth: '13rem'}}>Роль</th>
                        <th style={{minWidth: '13rem'}}>Имя</th>
                        <th style={{minWidth: '13rem'}}>Почта</th>
                        <th style={{minWidth: '13rem'}}>Телефон</th>
                        <th style={{minWidth: '13rem'}}>Действия</th>
                    </tr>
                    </thead>
                    <tbody>
                    {users && users.map(user => (
                        <tr key={user.id}>
                            <td style={{minWidth: '13rem'}}>{user.role}</td>
                            <td style={{minWidth: '13rem'}}>{user.full_name}</td>
                            <td style={{minWidth: '13rem', color: 'var(--green)'}}>{user.email_verified_at ? 'Подтверждена' : 'Не подтверждена'}</td>
                            <td style={{minWidth: '13rem'}}>{user.phone_number}</td>
