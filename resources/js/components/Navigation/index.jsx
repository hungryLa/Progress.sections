...
export const Navigation = ({ place, isAuthenticated = true }) => {
    ...
    useEffect(() => {
        ...
        if (
            user?.role === "user" &&
            schoolId &&
            location.pathname.split("/")[3] == schoolId
        ) {
        ...
        if (
            ...
            schoolId &&
            sectionId &&
            location.pathname.split("/")[5] == sectionId
        ) {
    ...
    return (
        <nav
            className={`navigation ${place ? "navigation-" + place : ""} ${
                !user ? "navigation-unauth" : ""
            }`}
        >
            <ul className={"navigation__list"}>
                {links.map((link) => (
                    <li className={"navigation__item"} key={link.title}>
                        ...
                        >
                            <svg
                                className={"navigation__item-image"}
                                width="30"
                                height="30"
                                viewBox="0 0 30 30"
                                fill="currentColor"
                ...
                {location.pathname.split('/')[4] == schoolId ?
                     (
